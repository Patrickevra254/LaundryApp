...
class ProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        // Notify admins/superAdmins — "John Doe updated their profile"
        // Skip if the user updating IS an admin (no need to notify yourself)
        if (!in_array($user->role, ['admin', 'superAdmin'])) {
            $admins = User::whereIn('role', ['admin', 'superAdmin'])->get();
            Notification::send($admins, new UserProfileUpdated($user, 'admin'));
        }

        // Always notify the user themselves — "Your profile was updated"
        $user->notify(new UserProfileUpdated($user, 'customer'));
