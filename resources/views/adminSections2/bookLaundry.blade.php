        ...
        <div class="col-12 col-lg-8">
            ...
            <div class="p-card mb-4">
                ...
                <div class="row g-3">
                    ...
                    @else
                        <div class="col-12">
                            <label class="p-label">Search Customer</label>

                            <div class="position-relative">
                                <div class="h-search-wrap">
                                    <i class="fa fa-search"></i>

                                    <input type="text" id="customerSearch" name="q" class="h-search"
                                        placeholder="Search by name or phone..." autocomplete="off"
                                        hx-get="{{ route('customers.search') }}"
                                        hx-trigger="keyup changed delay:300ms" hx-target="#customerResults"
                                        hx-swap="innerHTML">
                                </div>

                                <!--  Live Results Dropdown -->
                                <div id="customerResults" class="search-dropdown"></div>
                            </div>
            ...
            <div class="p-card mb-4">
                ...
                <div class="row g-3">
                    ...
                    @else
                        ...
                        <div class="col-12">
                            <label class="p-label">Search Customer</label>
                            <div class="cust-autocomplete-wrap" id="custAutocompleteWrap">
                                <i class="fa fa-search cust-search-icon"></i>
                                <input type="text" id="custSearchInput" class="p-input cust-search-input"
                                ...
                                <div class="cust-dropdown" id="custDropdown" style="display:none;"></div>
                            </div>
                        </div>

                        {{-- Selected customer card — shown after picking --}}
                        <div class="col-12" id="custSelectedCard" style="display:none;">
                            <div class="cust-selected">
                                <div class="cust-selected-avatar" id="custAvatar">?</div>
                                <div class="cust-selected-info">
                                    <div class="cust-selected-name" id="custSelectedName">—</div>
                                    <div class="cust-selected-meta" id="custSelectedMeta">—</div>
                                </div>
                                <span class="cust-selected-badge">
                                    <i class="fa fa-circle-check me-1"></i>Selected
                                </span>
                            </div>
                        </div>
                    @endif
            ...
            <div class="p-card mb-4">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12" id="partialRow">
                        ...
                        <div class="partial-input-wrap">
                            <span class="partial-prefix">₦</span>
                            <input type="number" id="amountPaidNow" class="p-input" min="0" value="0"
                                placeholder="0">
                        </div>
                        <div class="partial-hint">
                            Leave 0 to pay the <strong>full amount</strong>. Enter a partial amount to pay less now.
...
<script>
    ...
    function initCustomerAutocomplete() {
        ...
        function selectCustomer(id, name, phone, email) {
            ...
            // Show selected card
            const card = document.getElementById('custSelectedCard');
            if (card) card.style.display = '';

            const avatar = document.getElementById('custAvatar');
            if (avatar) avatar.textContent = name.charAt(0).toUpperCase();

            const nameEl = document.getElementById('custSelectedName');
        ...
        function clearSelection() {
            document.getElementById('field_customer_id').value = '';
            document.getElementById('field_customer_email').value = '';
        ...
        function showLoading() {
            ...
            dropdown.innerHTML =
                `<div class="cust-loading"><i class="fa fa-spinner fa-spin me-1"></i> Searching...</div>`;
        }

        function renderResults(customers) {
            if (!customers.length) {
                dropdown.innerHTML =
                    `<div class="cust-empty"><i class="fa fa-user-slash me-1"></i> No customers found</div>`;
                return;
            }
            dropdown.innerHTML = customers.map(c => `
                <div class="cust-item" data-id="${c.id}" data-name="${c.name}"
                    ...
                    <div>
                        ...
                        <div class="cust-item-phone">${c.phone || 'No phone'} · ${c.email || ''}</div>
                    </div>
                </div>
            `).join('');

            // Click to select
            dropdown.querySelectorAll('.cust-item').forEach(item => {
        ...
        input.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(() => {
                ...
                fetch(`{{ route('customers.search') }}?q=${encodeURIComponent(q)}`, {
                    ...
                    .then(res => res.json())
                    .then(customers => renderResults(customers))
                    .catch(() => {
                        dropdown.innerHTML = `<div class="cust-empty">Error loading results</div>`;
                    });
            }, 300);
        });
        ...
        document.addEventListener('click', function(e) {
            if (!document.getElementById('custAutocompleteWrap')?.contains(e.target)) {
                dropdown.style.display = 'none';
            }
        });
    ...
    document.addEventListener('htmx:afterSettle', initCustomerAutocomplete);
    document.addEventListener('DOMContentLoaded', initBookLaundry);
    document.addEventListener('htmx:afterSettle', initBookLaundry);
</script>
