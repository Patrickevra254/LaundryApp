        ...
        <div class="col-12 col-lg-8">
            ...
            <div class="p-card mb-4">
                ...
                <div class="row g-3">
                    ...
                    <!-- Partial amount — shown when "Pay Now" -->
...
{{-- <script>
    ...
    function updateSummaryPreview() {
        ...
        // 0 + Pay Now = full payment; 0 + On Delivery/At Store = nothing now
        const effectiveAmount = amountNow > 0 ?
            amountNow :
    ...
    function handleSubmit() {
        ...
        // Force recalculate all prices before reading total
        ...
        if (method === 'paystack') {
            // 0 means charge full total; partial > 0 means charge that amount
            ...
            // KEY FIX: if Pay Now and amount left as 0 → send full total (means pay in full)
            ...
            f('df_amount_paid_now').value = actualAmount; // ← sends full total when Pay Now + 0
    ...
    function initBookLaundry() {
        ...
        // Disable Add Item if no items exist in the database
        if (ITEMS.length === 0) {
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="detail-options" id="det_starch_opts">
                            <button type="button" class="detail-opt" data-val="none" data-charge="0">None</button>
                            <button type="button" class="detail-opt" data-val="low" data-charge="0">Low</button>
                            <button type="button" class="detail-opt active" data-val="medium"
                        ...
                        <div class="col-12">
                            <div class="det-charge-box">
                                <div class="det-charge-breakdown" id="det_charge_breakdown">
                                    <span class="det-charge-line" id="det_starch_line">Starch (Medium): ₦400</span>
                                    <span class="det-charge-sep">+</span>
                                    <span class="det-charge-line" id="det_heat_line">Heat (Medium): ₦200</span>
                                    <span class="det-charge-sep">+</span>
                                    <span class="det-charge-line" id="det_finish_line">Finish (Folded): ₦0</span>
                                </div>
                                ...
                                <div style="font-size:.72rem; color:#9ca3af; margin-top:4px;">
                                    Auto-calculated from selections above. You can override this amount manually.
                                </div>
                            </div>
                        </div>
                    @else
...
{{-- <script>
    ...
    function updateChargeBreakdown() {
        ...
        const starchCharge = parseInt(starchBtn?.dataset.charge || 0);
        const heatCharge = parseInt(heatBtn?.dataset.charge || 0);
        const finishCharge = parseInt(finishBtn?.dataset.charge || 0);
        ...
        if (sl) {
            sl.textContent = `Starch (${starchLabel}): ₦${starchCharge.toLocaleString()}`;
            sl.className = 'det-charge-line' + (starchCharge === 0 ? ' zero' : '');
        }
        if (hl) {
            hl.textContent = `Heat (${heatLabel}): ₦${heatCharge.toLocaleString()}`;
            hl.className = 'det-charge-line' + (heatCharge === 0 ? ' zero' : '');
        }
        if (fl) {
            fl.textContent = `Finish (${finishLabel}): ₦${finishCharge.toLocaleString()}`;
            fl.className = 'det-charge-line' + (finishCharge === 0 ? ' zero' : '');
        }

        // Auto-update the charge input (only if user hasn't manually overridden)
        const chargeInput = document.getElementById('det_extra_charge');
        if (chargeInput && !chargeInput.dataset.manualOverride) {
            chargeInput.value = calcPresetCharge();
        }
    }
